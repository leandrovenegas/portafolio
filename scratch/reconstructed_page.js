...
function VisualEditorContent() {
  ...
  const saveVersion = async (isExplicitNew = false) => {
    setSaving(true);
    setSaveSuccess('');
    setError('');

    try {
      const cv = versions.find(v => v.id === currentVersionId);
      const isCurrentlyPublished = cv?.is_published;
      
      // Auto-clone si guardamos borrador sobre algo publicado
      const isNew = isExplicitNew || (isCurrentlyPublished && !publishLock);
      
      const vName = isNew 
        ? (newVersionName || (isExplicitNew ? `v${versions.length + 1}` : `${cv?.version_name || 'v1'} (borrador)`))
        : cv?.version_name || 'v1';
      ...
      const payload = {
        ...
        components,
        is_active: true,
        is_published: publishLock // Candado define is_published
      };
