...
export async function fetchBunnyVideos() {
  ...
  try {
    ...
    try {
      ...
      configMap = new Map((configData.videos || []).map((item) => [item.videoId, item]));
    } catch (e) {
      console.warn('Failed to read videoConfig.json for overrides', e.message);
    }

    // Merge overrides before mapping so slugify picks up the clean title
    const mergedItems = items.map(video => {
        const videoId = getVideoId(video);
        const override = configMap.get(videoId) || {};
        return {
            ...video,
            title: override.title || video.title,
            description: override.description || video.description,
            thumbnailUrl: override.thumbnailUrl || video.thumbnailUrl,
        };
    });

    const normalized = mergedItems.map(mapVideo);
