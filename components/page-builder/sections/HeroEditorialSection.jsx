  ...
  return (
    ...
    >
      <div className="flex flex-col items-start gap-6 max-w-3xl">
        ...
        {/* Pill Tag */}
        {pillText && (
          <div className="px-3 py-1 rounded-full border border-s1/30 text-s1 text-xs tracking-widest uppercase mb-2">
            {pillText}
          </div>
        )}
        ...
        {(headline || headlineKeyword) && (
          <h1 className="font-display text-4xl md:text-5xl lg:text-7xl font-medium leading-[1.1] text-ink max-w-full">
            {headline}
            {headlineKeyword && <span className="text-accent"> {headlineKeyword}</span>}
          </h1>
        )}

        {/* Body Text */}
        {bodyText && (
          <p className="font-body text-lg md:text-xl leading-loose text-s1/90 max-w-2xl mt-4">
        ...
        <div className="flex flex-wrap items-center gap-6 mt-8">
          ...
          {secondaryButtonText && secondaryButtonLink && (
            <Link 
              ...
              className="flex items-center gap-2 text-s1 hover:text-ink transition-colors font-body font-medium"
            >
              {secondaryButtonText}
